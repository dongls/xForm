...
export function useInlineLayout(props: { field: FormField, disabled: boolean }, value: Ref<Row[]>){
  ...
  return function(){
        ...
        const indexSlots = {
          default(scope: { $index: number, row: any }){
            return [
              ...
              disabled ? null : <el-button onClick={removeRow.bind(null, scope.row)} type="danger" size="small" link class="xform-el-datatable-remove">删除</el-button>
            ]
          },
          header(){
            return (
              disabled ? '#' : <el-button onClick={addRow} type="primary" size="small" link>添加</el-button>
            )
          }
        }
    ...
    return <el-table data={value.value} v-slots={slots} size="small" class={klass} data-layout="inline"/>
  }
}
