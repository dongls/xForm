      ...
      const tabs = field.fields.map((f, index) => {
        ...
        return (
          <div class="xform-el-setting-option xform-el-tabs-option">
            <el-input placeholder="请输入标签名称" size="small" v-model={f.title}/>
            <el-button type="primary" size="small" plain onClick={up.bind(null, index)} disabled={index == 0}>上移</el-button>
            <el-button type="primary" size="small" plain onClick={down.bind(null, index)} disabled={index == length - 1}>下移</el-button>
            <el-button type="danger" size="small" plain onClick={removeTab.bind(null, f)} disabled={length <= 1}>删除</el-button>
          </div>
        )
      })
