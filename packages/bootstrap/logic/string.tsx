  ...
  render(logic: StringLogic, field){
    ...
    const value = (
      logic.operator === O.OPERATOR_EMPTY.type
        ? null
        : <input type='text' class="form-control form-control-sm" v-model={logic.value} placeholder="目标值"/>
    )
...
export const STRING_LENGTH_COMPARE = Field.createFieldLogic({
  ...
  render(logic: StringLogic, field){
    ...
    const value = (
      logic.operator === O.OPERATOR_EMPTY.type
        ? null
        : <input type='number' class="form-control form-control-sm" v-model={logic.value} placeholder="目标值"/>
    )
